...
import com.simple.download.DLTaskImp;
...
public class FtpDLTask extends DLTaskImp {

    public FtpDLTask(String downloadUrl, String filePath, int threadNum, DLListener dlListener) {
    ...
    public void run() {
        try {
            ...
            if (!FTPReply.isPositiveCompletion(reply)) {
                client.disconnect();
                DLLog.e("无法连接到ftp服务器，错误码为：" + reply);
                return;
            }
            // 读取下载文件总大小
