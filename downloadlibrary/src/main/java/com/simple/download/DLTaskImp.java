...
import com.simple.download.base.BreakInfo;
import com.simple.download.base.DLStringUtils;
import com.simple.download.base.DLSPUtils;
 ...
 * DLTaskImp 实现FTP/HTTP下载公共部分
 */

public abstract class DLTaskImp extends Thread implements DLTaskInf {
    protected String downloadUrl;  // 下载地址
    protected int threadNum;       //下载线程数
    protected String filePath;     // 下载文件路径
    private DLListener dlListener; // 下载进度监听
    ...
    public DLTaskImp(String downloadUrl, String filePath, int threadNum, DLListener dlListener) {
    ...
    public void startTimer() {
        ...
        TimerTask timerTask = new TimerTask() {
            ...
            public void run() {
                ...
                if (isFinished) {
                    DLSPUtils.remove(breakInfo.getFilePath());
                    timer.cancel();
                    ...
                    DLSPUtils.put(breakInfo.getFilePath(), breakInfo.toString());
                }
            }
        };
    ...
    protected boolean getBreakInfo() {
        ...
        try {
            String pathValue = (String) DLSPUtils.get(pathKey, " ");
            if (!DLStringUtils.isEmpty(pathValue)) {
